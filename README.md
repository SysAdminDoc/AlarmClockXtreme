...
![Version](https://img.shields.io/badge/version-1.15.23-blue)
...
```
adb install AlarmClockXtreme-v1.15.23-play-release.apk
```
...
| Bedtime Tracking | Sleep goal, sleep cycle calculator, bedtime reminders, guided breathing, sleep sounds, local room-noise baseline, and Android 16 final-hour bedtime countdown Live Updates |
...
| Feature | Description |
|---------|-------------|
| Backup/Restore | JSON export/import of all 50+ alarm fields and 60 settings, with optional AES-256 passphrase encryption, custom news-feed/background-image/manual-order/webhook-signing round-trip, pre-export disclosure for secrets/private references, and restore preview with append/replace/disabled import choices (v14 format) |
...
```bash
apksigner verify --print-certs AlarmClockXtreme-v1.15.23-play-release.apk
```
